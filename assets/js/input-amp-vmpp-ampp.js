(function($) {
  ...
  if (acf) {
    var AMPVMPPAMPP = acf.Field.extend({
      type: 'amp-vmpp-ampp',
      select2: false,
    ...
    acf.registerFieldType(AMPVMPPAMPP);
  }
  ...
  if (typeof acf.add_action !== 'undefined') {
    ...
    acf.add_action('ready append', function($el) {
      ...
      acf.get_fields({ type: 'amp-vmpp-ampp' }, $el).each(function() {
        initialize_field($(this));
      });
    });

    acf.add_filter('select2_args', function(args, $select, settings) {
      var parents = $select.parents('.acf-field-amp-vmpp-ampp');
      if (parents.length) {
        var original = null;
    ...
    $(document).on('acf/setup_fields', function(e, postbox) {
      $(postbox)
        .find('.field[data-field_type="amp-vmpp-ampp"]')
        .each(function() {
          initialize_field($(this));
        });
    });
